...
## Модуль finance.py
Модуль для чтения файлов формата csv и excel финансовых операция

| Метод                        | Аргументы                                 | Возвращаемый тип               | Описание                                                                                        |
|------------------------------|-------------------------------------------|--------------------------------|-------------------------------------------------------------------------------------------------|
| read_finance_csv_operation   | ```file_name``` тип str:  Строка          | list[dict]: Список словарей    | Функция для считывания финансовых операция из CSV и возвращает список словарей с транзакциями   |
| read_finance_excel_operation | ```file_path``` тип str:  Строка          | list[dict]: Список словарей    | Функция для считывания финансовых операция из Excel и возвращает список словарей с транзакциями |
# Тестирование
### Модуль masks
...
### Модуль finance
* Функция ```test_read_finance_csv_operation_basic```:
- Тестирование базового CSV файла
* Функция ```test_read_finance_csv_operation_empty```:
-Тестирование пустого CSV файла
*  Функция ```test_read_finance_excel_operation_basic```:
-Тестирование базового Excel файла
*  Функция ```test_read_finance_excel_operation_empty```:
-Тестирование пустого Excel файла