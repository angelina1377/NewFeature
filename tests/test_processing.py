...
@pytest.fixture
def x_data() -> List[Dict[str, Any]]:  # Добавлена типизация
    return [
...
@pytest.mark.parametrize(
    "state, result", [("EXECUTED", [41428829, 939719570]), ("CANCELED", [594226727, 615064591]), ("", [])]
)
def test_filter_by_state(x_data: List[Dict[str, Any]], state: str, result: list[int]) -> None:
    """
...
def test_sort_by_date(x_data: List[Dict[str, Any]]) -> None:  # "Функция сортировки данных по дате"
    sort_data: List[Dict[str, Any]] = sort_by_date(x_data)
    false_reverse_data: List[Dict[str, Any]] = sort_by_date(x_data, reverse=False)

    list_actual_ids: list[int] = [item["id"] for item in sort_data]
    list_false_actual_ids: list[int] = [item["id"] for item in false_reverse_data]
