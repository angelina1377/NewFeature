...
def test_file_not_exists():
    with patch.object(Path, "exists", return_value=False):  # Как будто файла не существует
        assert (
            load_operations("any/path.json") == []
        )  # При попытке загрузить несуществцющий файл должен возвращаться пустой список
...
def test_invalid_json_():
    ...
    with (
        ...
        patch.object(Path, "read_text", return_value=bad_json),
    ):  # Имитируем чтение некорректного файла
        assert load_operations("any/path.json") == []
...
def test_mixed_list_filters_non_dicts():  # имитация разных типов данных
    ...
    with patch.object(Path, "exists", return_value=True), patch.object(Path, "read_text", return_value=mixed):
        assert load_operations("any/path.json") == [
            {"a": 1},
            {"b": 2},
        ]  # Функция должна отфильтровать только словари и вернуть список словарей
...
def test_permission_error():
    ...
    with patch.object(Path, "exists", return_value=True), patch.object(Path, "read_text", side_effect=PermissionError):
        assert load_operations("any/path.json") == []
